...
from Code import load_materials, util, Model_Region
...
def main():
    ...
    ''' Load data '''
    arg_rootTrain = os.path.abspath('Data/Train_Region')
    arg_listTrain = './Data/list_trainf.txt'
    arg_batchsize_train= 12# 48

    arg_rooteval = './Data/Val_Region'
    arg_listeval = './Data/list_evalf.txt'
    arg_batchsize_eval= 8# 64

    train_loader, val_loader = load_materials.LoadAFEWRegion(arg_rootTrain, arg_listTrain, arg_batchsize_train, arg_rooteval, arg_listeval, arg_batchsize_eval)

    ''' Load model '''
    _structure = Model_Region.resnet18_AT(at_type=at_type)
    ...
    if args.evaluate == True:
        model= util.LoadBestModel(model)
...
def train(train_loader, model, criterion, optimizer, epoch):
    ...
    for i, ( img_copy, target_copy, img_first, target_first, img_second, target_second, img_third, target_third, img_fourth, target_fourth, img_fifth, target_fifth, index) in enumerate(train_loader):

        target = target_first.cuda(non_blocking=True)
        # print("input_var",input_var)
        input_var = torch.autograd.Variable(torch.stack([img_copy, img_first , img_second,img_third,img_fourth,img_fifth], dim=4))
...
def test(val_loader, model):
    ...
    with torch.no_grad():
        ...
        for i, (input_first, target_first, input_second, target_second, input_third, target_third, index) in enumerate(val_loader):
        ...
        prec_video = util.accuracy(pred_matrix_fc.cpu(), target_vector.cpu(), topk=(1,))
        topVideo.update(prec_video[0], i + 1)
        print('Validation *Prec@Video {topVideo.avg:.3f}   *Prec@Frame {topframe.avg:.3f} '.format(topVideo=topVideo, topframe=topframe))
        return topVideo.avg
