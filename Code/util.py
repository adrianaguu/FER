...
def save_checkpoint_region(state, at_type=''):

    if not os.path.exists('./model_region'):
        os.makedirs('./model_region')

    epoch = state['epoch']
    save_dir = './model_region/'+at_type+'_' + str(epoch) + '_' + str(round(float(state['prec1']), 4))
    torch.save(state, save_dir)
    print(save_dir)

def LoadBestModel(model):
    models = os.listdir('./model')
    models.sort()  # sort files by ascending
    print(models[0])
    checkpoint  = torch.load('./model/'+models[0])
